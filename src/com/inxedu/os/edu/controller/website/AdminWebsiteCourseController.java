...
public class AdminWebsiteCourseController extends BaseController {
	...
	private static final String updateWebsiteCourse = getViewPath("/admin/website/course/websiteCourse_update");//推荐课程分类修改页面
	private static final String addWebsiteCourse = getViewPath("/admin/website/course/websiteCourse_add");//推荐课程分类添加页面
	...
	public ModelAndView getWebsiteCourseList(HttpServletRequest request) {
		ModelAndView model = new ModelAndView(getWebsiteCourseList);
		try {
			// 查询模块课程分类
	...
	public ModelAndView getWebsiteCourse() {
		ModelAndView model = new ModelAndView();
		model.setViewName(addWebsiteCourse);
		return model;
	}
	/**
	 * 更新课程分类跳转
	...
	public ModelAndView doUpdateWebsiteCourse(HttpServletRequest request, @PathVariable("id") int id) {
		ModelAndView model = new ModelAndView();
		try {
			model.setViewName(updateWebsiteCourse);
			// 获得websiteCourse
